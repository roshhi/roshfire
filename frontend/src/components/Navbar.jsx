...
export default function Navbar({
  ...
  const goToWeapons = () =>
    weaponRef.current.scrollIntoView({ behavior: "smooth", block: "center" });
  const goToOrder = () =>
        ...
        {user ? (
          <div className="text-[rgb(10,10,10)] h-[100%] flex-center absolute right-4 lg:mr-[3.2vw]">
            <button
              ...
              className="bg-[#DFB159] border border-[#DFB159] h-[60%] w-[70px] rounded-[5px] text-white lg:w-[120px] lg:h-[80%] lg:p-1 lg:font-semibold xl:w-[150px] xl:h-[90%] xl:font-semibold xl:mediumResFont hover:bg-transparent hover:text-[#DFB159] transition-colors delay-50 "
            >
              Logout
            </button>
          </div>
      ...
      <div className=" relative h-[50px] border-t border-b border-[#9AA1AC] mt-8 flex items-center justify-evenly md:flex-center md:gap-[80px] lg:mt-11 lg:smallResFont xl:mediumResFont">
        {links.map((link) => {
          ...
          return (
            <div
              key={link.name}
              className={`group relative flex items-center gap-1 cursor-pointer transition-all duration-200 ${
            ...
            >
              ...
              {isHome && location.pathname === "/home" && (
                <div
                  className={`absolute w-[200px] top-[30px] right-[-65px] text-gray-400 p-2 rounded-lg bg-[rgb(10,10,10)] border-2 border-[#DFB159] hidden group-hover:block`}
                >
                  <div
                    onClick={goToWeapons}
                    className="flex items-center hover:text-[#DFB159] cursor-pointer"
                  >
