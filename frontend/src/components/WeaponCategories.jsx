    ...
    <div className="relative bottom-[60px] md:bottom-[70px] lg:bottom-[80px] xl:bottom-[100px]">
      <div className="flex items-center justify-center flex-col w-[90%] mx-auto bg-white">
        <div className="grid grid-cols-5 place-items-center w-[100%]  text-white extraSmallResFont h-[35px] font-medium">
          {categories.map((items) => {
            return (
              <p
                key={items.name}
                className={`h-[100%] w-[100%] flex-center transition-all duration-200 cursor-pointer md:smallResFont ${
                  selected === items.name
                    ? "bg-white text-[#DFB159]"
                    : "bg-[#DFB159]"
                }`}
        ...
        <motion.div
          ...
          className="weaponGrid grid grid-cols-3 gap-4 ml-4 bg-white pt-5 lg:grid-cols-6 mt-5"
        >
          {selected === "handgun" &&
            pistols.map((pistol) => {
              return (
                <motion.div
                  ...
                  key={pistol.name}
                  className="grid place-items-center border-2 hover:border-yellow-500 rounded-sm transform hover:translate-y-[-10px] transition-transform duration-300 hover:font-bold"
                >
